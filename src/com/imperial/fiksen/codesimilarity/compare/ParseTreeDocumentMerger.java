 ...
 */
public class ParseTreeDocumentMerger {
...
//		}
	...
	public void doDiff() throws CoreException {
		...
		final IDocument leftDoc = new Document(lDoc.get());
		final IDocument rightDoc = new Document(rDoc.get());
		...
		IRunnableWithProgress runnable= new IRunnableWithProgress() {
			public void run(IProgressMonitor monitor) throws InterruptedException, InvocationTargetException {
				...
				try {
					ParseTreeKernelSimilarityAnalyser analyser = new ParseTreeKernelSimilarityAnalyser();
					result[0]= analyser.compare(leftDoc, rightDoc);
		...
		try {
			getCompareConfiguration().getContainer().run(false, true, runnable);
