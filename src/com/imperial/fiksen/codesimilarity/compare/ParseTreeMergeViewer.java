...
import com.imperial.fiksen.codesimilarity.compare.ParseTreeDocumentMerger;
import com.imperial.fiksen.codesimilarity.compare.ParseTreeDocumentMerger.Diff;
import com.imperial.fiksen.codesimilarity.compare.ParseTreeDocumentMerger.IDocumentMergerInput;
 ...
 */
public class ParseTreeMergeViewer extends ContentMergeViewer implements
		IAdaptable {
	...
	private static final String[] TEXT_ACTIONS = { MergeSourceViewer.UNDO_ID,
			...
			MergeSourceViewer.FIND_ID, MergeSourceViewer.GOTO_LINE_ID };
	...
	private ParseTreeDocumentMerger fMerger;
	/** The current diff */
	...
	class ContributorInfo implements IElementStateListener, VerifyListener,
			IDocumentListener, IEncodingSupport {
		private final ParseTreeMergeViewer fViewer;
		...
		public ContributorInfo(ParseTreeMergeViewer viewer, Object element,
				char leg) {
			fViewer = viewer;
			fElement = element;
			fLeg = leg;
		 ...
		 * 
		 * @see
		 * org.eclipse.ui.editors.text.IEncodingSupport#setEncoding(java.lang
		 * .String)
		 */
		 ...
		 * 
		 * @see
		 * org.eclipse.ui.editors.text.IEncodingSupport#getDefaultEncoding()
		 */
		...
		public void setDocument(MergeSourceViewer viewer, boolean isEditable) {
			...
			if (isEditable) {
				...
				viewer.getSourceViewer().getTextWidget()
			...
			if (this.fViewer.fSubDoc) {
				if (range != null) {
					IRegion r = this.fViewer.normalizeDocumentRegion(tp
							...
							ParseTreeMergeViewer.toRegion(range));
					tp.getSourceViewer().setVisibleRegion(r.getOffset(),
							r.getLength());
				} else
			...
			if (this.fViewer.fSubDoc) {
				if (range != null) {
					IRegion r = this.fViewer.normalizeDocumentRegion(document,
							ParseTreeMergeViewer.toRegion(range));
					tp.getSourceViewer().setDocument(document, r.getOffset(),
							r.getLength());
				} else
		...
		public void disconnect() {
			...
			if (fSourceViewer != null
					&& !fSourceViewer.getSourceViewer().getTextWidget()
							.isDisposed()) {
				if (fNeedsValidation) {
					fSourceViewer.getSourceViewer().getTextWidget()
							.removeVerifyListener(this);
		...
		public void documentChanged(DocumentEvent e) {
			...
			ParseTreeMergeViewer.this.documentChanged(e, dirty);
	...
	private class FindReplaceTarget implements IFindReplaceTarget {
		...
		public int findAndSelect(int widgetOffset, String findString,
			...
			return fFocusPart
					...
					.findAndSelect(widgetOffset, findString, searchForward,
		...
		public void replaceSelection(String text) {
			fFocusPart.getSourceViewer().getFindReplaceTarget()
	 ...
	 */
	public ParseTreeMergeViewer(Composite parent,
			CompareConfiguration configuration) {
	 ...
	 *            the configuration object
	 */
	public ParseTreeMergeViewer(Composite parent, int style,
			CompareConfiguration configuration) {
	...
	protected void createControls(Composite composite) {
		...
		fAncestor.getSourceViewer().getTextWidget().getAccessible()
		...
		fAncestor.getSourceViewer().addTextPresentationListener(
				new ChangeHighlighter(fAncestor));
		...
		fLeft.getSourceViewer().getTextWidget().getVerticalBar()
				.setVisible(!fSynchronizedScrolling);
		fLeft.getSourceViewer().getTextWidget().getAccessible()
				.addAccessibleListener(new AccessibleAdapter() {
					public void getName(AccessibleEvent e) {
						e.result = NLS
		...
		fLeft.getSourceViewer().addTextPresentationListener(
				new ChangeHighlighter(fLeft));

		fRight = createPart(composite);
		fRight.getSourceViewer().getTextWidget().getVerticalBar()
				.setVisible(!fSynchronizedScrolling);
		fRight.getSourceViewer().getTextWidget().getAccessible()
				.addAccessibleListener(new AccessibleAdapter() {
					public void getName(AccessibleEvent e) {
						e.result = NLS
		...
		fRight.getSourceViewer().addTextPresentationListener(
				new ChangeHighlighter(fRight));
		...
		if (part != null) {
			...
			if (selectionProvider instanceof CompareEditorSelectionProvider) {
				...
				SourceViewer focusSourceViewer = fFocusPart == null ? null
						: fFocusPart.getSourceViewer();
				cesp.setViewers(
						new SourceViewer[] { fLeft.getSourceViewer(),
								fRight.getSourceViewer(),
								fAncestor.getSourceViewer() },
						focusSourceViewer);
			}
		}

		hsynchViewport(fAncestor.getSourceViewer(), fLeft.getSourceViewer(),
				fRight.getSourceViewer());
		hsynchViewport(fLeft.getSourceViewer(), fAncestor.getSourceViewer(),
				fRight.getSourceViewer());
		hsynchViewport(fRight.getSourceViewer(), fAncestor.getSourceViewer(),
				fLeft.getSourceViewer());
		...
		Rectangle trim = fLeft.getSourceViewer().getTextWidget()
				.computeTrim(0, 0, 0, 0);
		fTopInset = trim.y;
		...
		IWorkbenchPart workbenchPart = getCompareConfiguration().getContainer()
				.getWorkbenchPart();
	...
	private Diff getDiffUnderMouse(Canvas canvas, int mx, int my, Rectangle r) {
		...
		int lineHeight = fLeft.getSourceViewer().getTextWidget()
				.getLineHeight();
	...
	protected final Control createCenterControl(Composite parent) {
		if (fSynchronizedScrolling) {
			...
			if (fUseResolveUI) {
				...
				fLeftToRightButton.setCursor(new Cursor(canvas.getDisplay(),
						SWT.CURSOR_ARROW));
				fLeftToRightButton.setText(COPY_LEFT_TO_RIGHT_INDICATOR);
				fLeftToRightButton.setToolTipText(Utilities.getString(
						getResourceBundle(),
						"action.CopyDiffLeftToRight.tooltip")); //$NON-NLS-1$
				fLeftToRightButton.pack();
				fLeftToRightButton.setVisible(false);
				fLeftToRightButton.addSelectionListener(new SelectionAdapter() {
					public void widgetSelected(SelectionEvent e) {
				...
				fRightToLeftButton.setCursor(new Cursor(canvas.getDisplay(),
						SWT.CURSOR_ARROW));
				fRightToLeftButton.setText(COPY_RIGHT_TO_LEFT_INDICATOR);
				fRightToLeftButton.setToolTipText(Utilities.getString(
						getResourceBundle(),
						"action.CopyDiffRightToLeft.tooltip")); //$NON-NLS-1$
				fRightToLeftButton.pack();
				fRightToLeftButton.setVisible(false);
				fRightToLeftButton.addSelectionListener(new SelectionAdapter() {
					public void widgetSelected(SelectionEvent e) {
		...
		if (diff != fButtonDiff) {
			if (diff != null) {
				...
				if (leftEditable && rightEditable) {
					...
					Rectangle leftToRightBounds = new Rectangle(r.x,
							leftToRightY, r.width, r.height);
					fLeftToRightButton.setBounds(leftToRightBounds);
					fLeftToRightButton.setVisible(true);
					Rectangle rightToLeftBounds = new Rectangle(r.x,
							rightToLeftY, r.width, r.height);
	 ...
	 * {@link #createControls(Composite)}). It is called three times for each
	 * text part of the comparison: ancestor, left, right. Clients may implement
	 * to provide their own type of source viewers. The viewer is not expected
	 * to be configured with a source viewer configuration.
	 * 
	 * @param parent
	...
	protected SourceViewer createSourceViewer(Composite parent,
			int textOrientation) {
		return new SourceViewer(parent, new CompositeRuler(), textOrientation
	...
	protected IEditorInput getEditorInput(ISourceViewer sourceViewer) {
		...
		if (fAncestor != null && sourceViewer == fAncestor.getSourceViewer())
	...
	private MergeSourceViewer createPart(Composite parent) {
		...
		try{
			new JavaTextTools(fPreferenceStore).setupJavaDocumentPartitioner(viewer.getRememberedDocument());
		} catch(NullPointerException e) {//thrown when the doc isn't a java doc
			//FIXME
		}
	...
	private void contributeFindAction(MergeSourceViewer viewer) {
		...
		if (wp != null)
			...
			action = new FindReplaceAction(
					getResourceBundle(),
					"Editor.FindReplace.", viewer.getSourceViewer().getControl().getShell(), getFindReplaceTarget()); //$NON-NLS-1$
	...
	private void contributeGotoLineAction(MergeSourceViewer viewer) {
		IAction action = new GotoLineAction(
				(ITextEditor) viewer.getAdapter(ITextEditor.class));
	...
	private void connectContributedActions(final MergeSourceViewer viewer,
	 ...
	 * <p>
	 * The default implementation does nothing, but clients should override to
	 * properly react to viewers switching.
	 * </p>
	 * 
	 * @param sourceViewer
	 *            the source viewer
...
//XXX:HERE!
		...
		if (!isConfigured) {
			...
			configureSourceViewer(fLeft.getSourceViewer(), cc.isLeftEditable()
					&& cp.isLeftEditable(input), fLeftContributor);
		...
		if (!fHasErrors && !emptyInput && !getControl().isDisposed()) {
			if (isRefreshing()) {
				fLeftContributor
	...
	private void configureSourceViewer(SourceViewer sourceViewer,
			boolean editable, ContributorInfo contributor) {
				...
				if (contents != null) {
					...
					try {
						bytes = contents.getBytes(left ? fLeftContributor
								.internalGetEncoding() : fRightContributor
								.internalGetEncoding());
	...
	protected final void handleResizeLeftRight(int x, int y, int width1,
		...
		if (fSynchronizedScrolling && fScrollCanvas != null) {
			trim = fLeft.getSourceViewer().getTextWidget()
					.computeTrim(0, 0, 0, 0);
	...
	private void doDiff() {
		...
		try {
			...
			String msg = Utilities.getString(getResourceBundle(),
					"tooComplexError.message"); //$NON-NLS-1$
			MessageDialog.openError(getControl().getShell(), title, msg);
		}
	...
	protected void createToolItems(ToolBarManager tbm) {
		...
		fHandlerService = CompareHandlerService.createFor(
				getCompareConfiguration().getContainer(), fLeft
						.getSourceViewer().getControl().getShell());
		...
		boolean needsAncestorPainter = !isEditorBacked(fAncestor
				.getSourceViewer());
		showWhitespaceAction = new ShowWhitespaceAction(
				new MergeSourceViewer[] { fLeft, fRight, fAncestor },
				new boolean[] { needsLeftPainter, needsRightPainter,
						needsAncestorPainter });
	...
	private void selectFirstDiff(boolean first) {
		...
		if (fLeft.getSourceViewer().getDocument() == null
	...
	private void paintCenter(Canvas canvas, GC g) {
		...
		int lineHeightLeft = fLeft.getSourceViewer().getTextWidget()
				.getLineHeight();
		int lineHeightRight = fRight.getSourceViewer().getTextWidget()
				.getLineHeight();
		int visibleHeight = fRight.getViewportHeight();
	...
	private void handleEndOfDocumentReached(Shell shell, boolean next) {
		...
		if (!value.equals(ICompareUIConstants.PREF_VALUE_PROMPT)) {
			...
			String[] doNothingOption = new String[] {
					CompareMessages.TextMergeViewer_17,
			...
			NavigationEndDialog dialog = new NavigationEndDialog(shell, title,
					null, message, new String[][] { localLoopOption,
							nextElementOption, doNothingOption });
	...
	private void revealDiff(Diff d, boolean smart) {
		...
		if (smart) {
			...
			if (isThreeWay() && !isIgnoreAncestor()) {
				...
				if (as >= fAncestor.getSourceViewer().getTopIndex()
						&& as <= fAncestor.getSourceViewer().getBottomIndex())
					ancestorIsVisible = true;
			}

			if (ls >= fLeft.getSourceViewer().getTopIndex()
					&& ls <= fLeft.getSourceViewer().getBottomIndex())
				leftIsVisible = true;

			if (rs >= fRight.getSourceViewer().getTopIndex()
					&& rs <= fRight.getSourceViewer().getBottomIndex())
				rightIsVisible = true;
		}
		...
		if (!leftIsVisible || !rightIsVisible) {
			...
			if (leftIsVisible) {
				avpos = lvpos = rvpos = realToVirtualPosition(LEFT_CONTRIBUTOR,
						fLeft.getSourceViewer().getTopIndex());
				allButThis = fLeft;
			} else if (rightIsVisible) {
				avpos = lvpos = rvpos = realToVirtualPosition(
						RIGHT_CONTRIBUTOR, fRight.getSourceViewer()
								.getTopIndex());
				allButThis = fRight;
			} else if (ancestorIsVisible) {
				avpos = lvpos = rvpos = realToVirtualPosition(
						ANCESTOR_CONTRIBUTOR, fAncestor.getSourceViewer()
								.getTopIndex());
	...
	void copyAllUnresolved(boolean leftToRight) {
		if (fMerger.hasChanges() && isThreeWay() && !isIgnoreAncestor()) {
			IRewriteTarget target = leftToRight ? fRight.getSourceViewer()
					.getRewriteTarget() : fLeft.getSourceViewer()
					.getRewriteTarget();
	...
	protected void copy(boolean leftToRight) {
		...
		if (leftToRight) {
			...
			if (fRight.getEnabled()) {
				// copy text
				String text = fRight.getSourceViewer().getTextWidget()
						.getText();
	...
	private ITextEditor getTextEditorAdapter() {
		return new ITextEditor() {
			...
			private IEncodingSupport getEncodingSupport(
				...
				if (contributor != null
						&& contributor.getDefaultEncoding() != null) {
					return contributor;
				}
				return null;
			}
...
// ASTParser parser = ASTParser.newParser(AST.JLS4); // handles JDK 1.0, 1.1,
// 1.2, 1.3, 1.4, 1.5, 1.6
...
// }
//
// //return true if they're dissimilar as then we don't want them
// highlight/linked
// @Override
...
// }
//
// double comp = ParseTreeKernelSimilarityAnalyser.calculateK(thisNode,
// otherNode);
// double same1 = ParseTreeKernelSimilarityAnalyser.calculateK(thisNode,
// thisNode);
// double same2 = ParseTreeKernelSimilarityAnalyser.calculateK(otherNode,
// otherNode);
// double similarity =
// ParseTreeKernelSimilarityAnalyser.normaliseSimilarity(comp, same1, same2);
//
